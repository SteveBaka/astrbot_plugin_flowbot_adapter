...
### 修复：平台状态显示
- `get_stats()` 改为先取基类契约字段再合并本适配器计数。此前整体覆盖为
  `{"recv", "sent"}`，丢掉了 `status` / `started_at` / `meta` 等字段，
  Dashboard 取不到状态值，平台卡片恒显示「未知」（灰点），而实际连接与
  收发消息均正常。本适配器计数仍以 `recv` / `sent` 增量保留。

### 新增：适配器 Logo
- `@register_platform_adapter` 补上 `logo_path="logo.png"`，Bot 列表卡片
  显示本插件 logo，替换此前的通用图标。该字段按适配器独立注册进
  `config_template[<适配器名>]`，不共享、不写入其他适配器的条目。

## v1.4.2 — 2026-09-14

### 新增：机器人身份链路（self_id 端到端）
- 启动预热：经 FlowBot Bot Token 鉴权路由主动获取登录 wxid，消除重启后到
  首条推送之间的身份盲区（该窗口内 @ 唤醒与引用 is_self 判定失效）。
